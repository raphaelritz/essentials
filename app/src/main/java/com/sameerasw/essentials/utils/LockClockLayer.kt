...
class LockClockLayer(
    ...
    companion object {
        ...
        /**
         * The framework sends the wake command as waking starts; the keyguard's own slide begins
         * this much later, measured from its face's positions in its tree over three wakes.
         */
        private const val KEYGUARD_WAKE_LAG_MS = 100L
        ...
        fun swapElapsed(eased: Float): Long {
            var low = 0f
            var high = 1f
            repeat(16) {
                val mid = (low + high) / 2f
                if (SWAP_INTERPOLATOR.getInterpolation(mid) < eased) low = mid else high = mid
            }
            return (high * SWAP_MS).toLong()
        }
    }
    ...
    /** The always-on display shows the plugin's outline, which the wallpaper clock can keep on the lock screen. */
    private var outline = repository.getLockClockOutline()
    ...
    fun onPreferenceChanged(key: String) {
        when (key) {
            ...
            SettingsRepository.KEY_LOCK_CLOCK_OUTLINE -> {
                outline = repository.getLockClockOutline()
    ...
    fun wake(
        ...
        // A dark display, still off when the wake command arrives, showed no always-on face: the
        // keyguard then wakes the clock at rest, its doze animation done while the panel lights up.
     ...
     */
    private fun startDozeAnimation(lag: Long) {
        placeAod()
        var frames = 0
        var last = -1f
        val animator =
        ...
        handler.postDelayed(animator::start, lag)
        invalidate()
    }
    ...
    private fun place() {
        ...
        ClockLog.add(context, "place: ${if (small) "small" else "large"} at ${rect.toShortString()}${if (rect.isEmpty) " (unmeasured)" else ""}")
    }
    ...
    private fun drawDozeTransition(
        ...
        val fade = if (fallingAsleep) 1f - linear else 1f
